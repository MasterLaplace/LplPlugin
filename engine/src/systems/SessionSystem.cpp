...
static const ecs::SystemDescriptor kSessionDesc{
    "Session",
...
struct SessionSystem::Impl
{
    ...
    Impl(net::session::SessionManager& sm, EventQueues& q,
         std::shared_ptr<net::transport::ITransport> t,
         input::InputManager& im, ecs::WorldPartition& w,
         ecs::Registry& reg)
        : sessionManager{sm}, queues{q}, transport{std::move(t)},
          inputManager{im}, world{w}, registry{reg}
    {
    }
};
...
SessionSystem::SessionSystem(
    ...
    ecs::WorldPartition& world,
    ecs::Registry& registry)
    : _impl{std::make_unique<Impl>(sessionManager, queues, std::move(transport),
                                    inputManager, world, registry)}
{
}

SessionSystem::~SessionSystem() = default;

const ecs::SystemDescriptor& SessionSystem::descriptor() const noexcept
{
    return kSessionDesc;
}
...
void SessionSystem::execute(core::f32 /*dt*/)
{
    ...
    for (const auto& ev : events)
    {
        ...
        if (ref.chunkIndex < static_cast<core::u32>(chunks.size()))
        {
            ...
            // SleepState: awake
            if (auto* ws = static_cast<core::u8*>(
                    chunk.writeComponent(ecs::ComponentId::SleepState)))
                ws[ref.localIndex] = 0;
        }
        ...
        [[maybe_unused]] auto sendRes = _impl->transport->send(
            std::span<const core::byte>{packet, sizeof(packet)},
            session->address());
