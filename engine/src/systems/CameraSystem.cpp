...
static const ecs::SystemDescriptor kCameraSystemDesc{
    "CameraSystem",
...
static bool getEntityPosition(ecs::Registry& registry, core::u32 id, math::Vec3<float>& outPos)
{
    ...
    for (const auto& part : partitions)
    {
        ...
        if (ref.chunkIndex >= static_cast<core::u32>(chunks.size())) continue;
        ...
        for (core::u32 i = 0; i < chunk.count(); ++i)
        {
            ...
            {
                if (auto* rpos = static_cast<const math::Vec3<float>*>(
...
CameraSystem::CameraSystem(CameraData& cameraData,
...
const ecs::SystemDescriptor& CameraSystem::descriptor() const noexcept
{
    return kCameraSystemDesc;
}
...
void CameraSystem::execute(core::f32 dt)
{
    ...
    if (glfwGetKey(_window, GLFW_KEY_UP) == GLFW_PRESS)
    {
        ...
        if (_cameraData.pitch > 89.f) _cameraData.pitch = 89.f;
        updateCameraDirection(_cameraData);
    }
