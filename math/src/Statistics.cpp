...
double Statistics::integratePsd(std::span<const double> psd, core::u32 binLow, core::u32 binHigh)
{
    double sum = 0.0;
...
void Statistics::slidingWindowRms(std::span<const double> signal, core::u32 windowSize, std::span<double> rms)
{
...
void Statistics::computeBaseline(std::span<const double> samples, double &mean, double &stddev)
{
    if (samples.empty())
    {
        mean = 0.0;
