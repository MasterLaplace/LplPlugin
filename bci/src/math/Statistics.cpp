...
float Statistics::integratePsd(std::span<const float> psd, std::size_t lowerBin, std::size_t upperBin) noexcept
{
    if (psd.empty() || lowerBin > upperBin || upperBin >= psd.size())
        return 0.0f;

    return std::accumulate(psd.begin() + static_cast<std::ptrdiff_t>(lowerBin),
                           psd.begin() + static_cast<std::ptrdiff_t>(upperBin) + 1, 0.0f);
}

std::size_t Statistics::hzToBin(float hz, float sampleRate, std::size_t fftSize) noexcept
{
...
float Statistics::slidingWindowRms(std::span<const float> data, std::size_t windowSize) noexcept
{
...
Baseline Statistics::computeBaseline(std::span<const float> data) noexcept
{
    if (data.empty())
