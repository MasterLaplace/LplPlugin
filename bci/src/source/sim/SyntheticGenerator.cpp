...
SyntheticGenerator::SyntheticGenerator(std::uint64_t seed, std::size_t channelCount)
    : _profile{}, _sampleIndex(0), _blinkRemaining(0), _channelCount(channelCount)
{
    if (seed == 0)
    {
...
const SyntheticProfile &SyntheticGenerator::profile() const noexcept { return _profile; }
...
std::vector<Sample> SyntheticGenerator::generate(std::size_t count)
{
    ...
    for (std::size_t t = 0; t < count; ++t)
    {
        ...
        {
            _blinkRemaining = static_cast<int>(_profile.blinkDurationSec * kDefaultSampleRate);
        }
        ...
        if (_blinkRemaining > 0)
        {
            ...
            blinkEnvelope =
                _profile.blinkAmplitudeUv * 0.5f * (1.0f - std::cos(2.0f * std::numbers::pi_v<float> * progress));
            --_blinkRemaining;
        }
        ...
        for (std::size_t ch = 0; ch < _channelCount; ++ch)
        {
            ...
            {
                const float phase = 2.0f * std::numbers::pi_v<float> * osc.freqHz * timeSec + osc.phaseOffset +
            ...
            const float blinkWeight = (ch < 2) ? 1.0f : (ch < 4) ? 0.3f : 0.05f;
...
std::uint64_t SyntheticGenerator::sampleIndex() const noexcept { return _sampleIndex; }
