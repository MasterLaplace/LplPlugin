...
FftProcessor::FftProcessor(std::size_t fftSize)
    : _fftSize(fftSize), _halfSize(fftSize / 2), _normFactor(2.0f / static_cast<float>(fftSize))
{
...
Expected<SignalBlock> FftProcessor::process(const SignalBlock &input)
{
    if (input.empty())
    {
        return std::unexpected(Error::make(ErrorCode::kEmptyInput, "FftProcessor received empty block"));
    }

    if (input.sampleCount() != _fftSize)
    {
        return std::unexpected(
            Error::make(ErrorCode::kFftSizeMismatch, "FftProcessor expects " + std::to_string(_fftSize) +
...
std::string_view FftProcessor::name() const noexcept { return "FftProcessor"; }
