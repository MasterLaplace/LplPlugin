...
StabilityMonitorBox::StabilityMonitorBox(const StabilityMonitorConfig &config)
    : _config(config), _metric(metric::StabilityConfig{
...
Expected<StabilityMonitorResult> StabilityMonitorBox::update(const Eigen::MatrixXf &covariance)
{
    auto result = _metric.update(covariance);
    if (!result)
        return std::unexpected(result.error());

    const float normalizedDist = std::min(result->currentDistance / _config.maxExpectedDist, 1.0f);
    const float instantStability = 1.0f - normalizedDist;

    _smoothedStability =
        _smoothedStability * (1.0f - _config.smoothingFactor) + instantStability * _config.smoothingFactor;
...
float StabilityMonitorBox::stability() const noexcept { return _smoothedStability; }
