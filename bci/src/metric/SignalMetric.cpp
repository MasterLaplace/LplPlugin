...
std::vector<SignalMetricResult> SignalMetric::compute(std::span<const std::vector<float>> psd) const noexcept
{
...
float SignalMetric::computeMean(std::span<const std::vector<float>> psd) const noexcept
{
    ...
    const float sum = std::accumulate(results.begin(), results.end(), 0.0f,
