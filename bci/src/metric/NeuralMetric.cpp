...
NeuralMetric::NeuralMetric(const NeuralMetricConfig &config) : _config(config) {}
...
NeuralState NeuralMetric::compute(std::span<const float> alphaPowers, std::span<const float> betaPowers) const noexcept
{
    ...
    for (std::size_t ch = 0; ch < channelCount; ++ch)
    {
        ...
        state.channelAlpha[ch] = normalize(alphaPowers[ch], bl.alpha.mean, bl.alpha.stdDev, _config.kSigma);
        state.channelBeta[ch] = normalize(betaPowers[ch], bl.beta.mean, bl.beta.stdDev, _config.kSigma);
...
float NeuralMetric::normalize(float value, float mean, float stdDev, float kSigma) noexcept
{
